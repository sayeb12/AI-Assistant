...
def graph_coloring(tasks, constraints, time_slots):
    ...
    for task in tasks:
        ...
        if available_colors:
            ...
            result[task] = time_slots[chosen_color]
        else:
    ...
    while True:
        ...
        if option == 1:
            ...
            for i in range(num_tasks):
                ...
                duration = int(input(f"Enter duration (in minutes) for {task_name}: "))
            ...
            num_slots = int(input("Enter the number of available time slots: "))
            time_slots = []
            for i in range(num_slots):
                start_time = input(f"Enter start time for time slot {i + 1} (HH:MM): ")
                end_time = input(f"Enter end time for time slot {i + 1} (HH:MM): ")
                time_slots.append(f"{start_time} - {end_time}")

            schedule = graph_coloring(tasks, constraints, time_slots)
